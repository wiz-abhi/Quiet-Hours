...
- **RTS detection loop** (`src/detection/`). RTS is the agent's sensory system: cross-message, permission-aware context — who's speaking, how often, when anyone else last replied — the kind of judgment you cannot make from a single event payload. `rtsClient.js` attempts the Real-Time Search path first and, wherever a bot token's scopes or a missing per-event `action_token` can't reach it, falls back to `conversations.history` for the same channel context. We built that fallback deliberately, not as an afterthought: detection must never go dark just because one permission boundary was hit, so the watcher always has a working source of live channel context feeding `heuristic.js`, the 4-signal check. Remove this layer entirely and the agent goes blind.
